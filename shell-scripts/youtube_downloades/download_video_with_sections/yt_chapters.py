...
#how to install :
    ...
    #it needs youtube-dl (make sure to download it with pip3 not apt-get otherwise it won't work)
        #sudo apt purge youtube-dl 
        #sudo pip3 install youtube-dl
    #it needs package called gpac which containse MP4Box command needed in this script
        #sudo apt-get install -y gpac

    #example usage : yt_chapters.py --split "https://www.youtube.com/watch?v=m5lp8S-YgrQ"
    #incase you want to run it from current directory --> sudo python yt_chapters.py --split "https://www.youtube.com/watch?v=k5E2AVpwsko"
...
def read_description_file(description_file:str)->list:
    ...
    with open(description_file, 'r') as f:
        ...
        for line in f:
            ...
            result = re.search(r"([0-1]?\d|2[0-3])(?::([0-5]?\d))?(?::([0-5]?\d))?", line)


            try:
                # print('resut ----->', result)
                # print('group(1) ----->', result.group(2))
                format1 = f'{result.group(1)}:{result.group(2)}:{result.group(3)}'
                # print('format1 ----->', format1)
                time_count = datetime.datetime.strptime(format1, '%H:%M:%S')               
                print('time1 ----->', format1)
            except:
                try:

                    format2 = f'{result.group(1)}:{result.group(2)}'
                    time_count = datetime.datetime.strptime(format2, '%M:%S')
                    print('time2 ----->', format2)
                    # print('time2 ----->', result)
...
def split_mp4(chapters:list, download_filename:str, download_name:str)->None:
    ...
    # print(download_name )        
    # print(current_duration_pretext.stdout[18:-13])
    # print('------------------------')
    ...
    for current_index, current_chapter in enumerate(chapters):
        ...
        safe_name = "".join([c for c in current_chapter[2] if c.isalpha() or c.isdigit() or c==' ']).rstrip()
        output_name = f'{current_index}_{safe_name}.mp4'
